...
return function (ui, terrain)
    ...
    if ui:windowBegin('Main Menu', widget_width_padding, widget_height_padding, widget_width, widget_height) then
		...
		ui:spacing(1)
        ui:layoutRow('dynamic', 32, 1)
		CHAR_NB = math.floor(ui:property('Personnages', 2, CHAR_NB, 10, 1, 0.2))
        ui:layoutRow('dynamic', 5, 1)
		ui:spacing(1)
        ui:layoutRow('dynamic', 32, 1)
		TEAM_NB = math.floor(ui:property('Equipes', 2, TEAM_NB, 6, 1, 0.2))
        ui:layoutRow('dynamic', 5, 1)
		ui:spacing(1)
        ui:layoutRow('dynamic', 32, 1)
		CHAR_HP = math.floor(ui:property('Vie', 50, CHAR_HP, 200, 10, 0.1))
        ui:layoutRow('dynamic', 5, 1)
		ui:spacing(1)
        ui:layoutRow('dynamic', 32, 1)
		TOUR_TIME = math.floor(ui:property('Temps', 10, TOUR_TIME, 60, 1, 0.2))
        ui:layoutRow('dynamic', 5, 1)
		ui:spacing(1)
        ui:layoutRow('dynamic', 32, 1)
